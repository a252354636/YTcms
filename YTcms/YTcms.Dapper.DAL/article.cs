...
namespace YTcms.Dapper.DAL
{
  ...
  public partial class article
  {
    ...
    {
      using (var conn = new SqlConnection(DbHelperSQL.connectionString))
      {
        conn.Open();
        ...
        return conn.Execute(strSql.ToString(), new { article_id = article_id }) > 0;
      }
    }
    ...
    public bool Exists(string channel_name, string call_index)
    {
      using (var conn = new SqlConnection(DbHelperSQL.connectionString))
      {
        ...
        return conn.Execute(strSql.ToString(), new { call_index = call_index }) > 0;
      }
    }
    ...
    /// 增加一条数据
    /// </summary>
    public int Add(YTcms.Dapper.Model.article model)
    {
      //查询频道名称
      ...
      using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
      {
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
          try
          {
            ...
            object obj = conn.Execute(strSql.ToString(), paras, trans);//带事务
      ...
      using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
      {
        ...
        using (SqlTransaction trans = conn.BeginTransaction())
        {
          try
          {
            ...
            conn.Execute(strSql.ToString(), paras, trans);
            #endregion
