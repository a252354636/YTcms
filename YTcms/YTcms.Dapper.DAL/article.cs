...
namespace YTcms.DAL
{
	...
	public partial class article
	{
		...
		/// 增加一条数据
		/// </summary>
        public int Add(Dapper.Model.article model)
        {
            //查询频道名称
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            foreach (Dapper.Model.user_group_price modelt in model.group_price)
                            {
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Dapper.Model.article model)
		{
            //查询频道名称
            string channelName = new DAL.site_channel(databaseprefix).GetChannelName(model.channel_id);
            if (channelName.Length == 0)
		...
		{
            //取得相册MODEL
            List<Dapper.Model.article_albums> albumsList = new DAL.article_albums(databaseprefix).GetList(channel_id, article_id);
            //取得附件MODEL
            List<Dapper.Model.article_attach> attachList = new DAL.article_attach(databaseprefix).GetList(channel_id, article_id);
		...
		/// 得到一个对象实体
		/// </summary>
        public Dapper.Model.article GetModel(string channel_name, int article_id)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Dapper.Model.article GetModel(string channel_name, string call_index)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 * from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where call_index=@call_index");
        ...
        /// 将对象转换为实体
        /// </summary>
        private Dapper.Model.article DataRowToModel(DataRow row)
        {
            Dapper.Model.article model = new Dapper.Model.article();//主表字段
            if (row != null)
            {
                #region 主表信息======================
                //利用反射获得属性的所有公共属性
